...
from applications import ApplicationStates
from constants import (SCREEN_WIDTH, SCREEN_HEIGHT, HEADER_WIDTH, HEADER_HEIGHT, MAIN_MENU_BUTTON_WIDTH, BUTTON_WIDTH, BUTTON_HEIGHT, ENTRY_WIDTH, ENTRY_HEIGHT, USER_BUTTON_WIDTH, USER_BUTTON_HEIGHT)
from database import (add_service, add_user, check_data_from_service, create_database, delete_service, delete_user, get_master_hashed, get_key, get_user_id, get_usernames_list, list_saved_services, update_service_password, update_service_username)
from encryption import get_hash, generate_key, encrypt_password, decrypt_password
...
class Application(object):
    ...
    def setupUi(self, MainWindow):
        ...
        #Enter info button
        self.enter_data = QPushButton(self.centralwidget)
        font.setPointSize(16)
        self.enter_data.setFont(font)
        self.enter_data.hide()
        self.enter_data.clicked.connect(self.enter)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.enter_data.setText(_translate('MainWindow', 'Enter'))
    ...
    def reset_widgets(self):
        ...
        enter_data_button_height = show_hide_password_button_height - SCREEN_HEIGHT // 10
        self.enter_data.setGeometry(
            (SCREEN_WIDTH - BUTTON_WIDTH) // 2, enter_data_button_height,
            BUTTON_WIDTH, BUTTON_HEIGHT)
        ...
        if self.state == ApplicationStates.SIGN_UP:
            ...
            if self.provided_username == '' or self.provided_password == '':
                self.warning.setText(
                    'You must type a username and a password!')
                self.warning.show()
                return
            ...
            for name in users_list:
                ...
                try:
                    ...
                    self.warning.setText('A problem occurred, try again!')
                    self.warning.show()
            ...
            if service_name == '' or (username == '' and password == ''):
                self.warning.setText(
                    'You must choose the service name and one of the others!')
                self.warning.show()
                return
